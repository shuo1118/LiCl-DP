...
为了构建高质量的LiCl熔体深度势能模型的训练数据集，我们可以使用DP-GEN软件。DP-GEN是一个自动化的训练数据集生成工具，可以根据预设的参数和条件搜索材料的结构和相态空间。学习完本课程后你应该：
...
在本教程中，我们以LiCl熔体分子为例。我们已经在`ex4`中准备了初始数据。

```sh
git clone XXX
```

首先，使用 `tree` 命令查看 `ex4` 文件夹。
